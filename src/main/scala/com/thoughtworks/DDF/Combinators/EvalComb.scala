...
trait EvalComb extends EvalArrow with Comb[Loss, Eval] {
  override def S[A, B, C](implicit at: Loss[A], bt: Loss[B], ct: Loss[C]): Eval[(A => B => C) => (A => B) => A => C] =
    arrEval[
      ...
      abc => (arrEval[A => B, A => C, ArrowLoss[A, bt.loss], ArrowLoss[A, ct.loss]](
  ...
  override def K[A, B](implicit at: Loss[A], bt: Loss[B]): Eval[A => B => A] =
    arrEval[A, B => A, at.loss, ArrowLoss[B, at.loss]](a => (
  ...
  override def Y[A, B](implicit at: Loss[A], bt: Loss[B]): Eval[((A => B) => A => B) => A => B] = {
    ...
    arrEval[(A => B) => A => B, A => B, ArrowLoss[A => B, abt], abt](abab => {
  ...
  override def B[A, B, C](implicit ai: Loss[A], bi: Loss[B], ci: Loss[C]): Eval[(B => C) => (A => B) => A => C] =
    arrEval[B => C, (A => B) => A => C, ArrowLoss[B, ci.loss], ArrowLoss[A => B, ArrowLoss[A, ci.loss]]](bc =>
      (arrEval[A => B, A => C, ArrowLoss[A, bi.loss], ArrowLoss[A, ci.loss]](ab => (arrEval[A, C, ai.loss, ci.loss](a => {
        val b = aeval(ab).forward(a)
  ...
  override def C[A, B, C](implicit ai: Loss[A], bi: Loss[B], ci: Loss[C]): Eval[(A => B => C) => B => A => C] =
    arrEval[A => B => C, B => A => C, ArrowLoss[A, ArrowLoss[B, ci.loss]], ArrowLoss[B, ArrowLoss[A, ci.loss]]](abc =>
      (arrEval[B, A => C, bi.loss, ArrowLoss[A, ci.loss]](b =>
  ...
  override def W[A, B](implicit ai: Loss[A], bi: Loss[B]): Eval[(A => A => B) => A => B] =
    arrEval[A => A => B, A => B, ArrowLoss[A, ArrowLoss[A, bi.loss]], ArrowLoss[A, bi.loss]](aab =>
