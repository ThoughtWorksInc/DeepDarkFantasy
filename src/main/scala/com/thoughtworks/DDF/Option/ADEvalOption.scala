...
trait ADEvalOption extends Option[ADEvalCase, ADEval] with ADEvalArr {
  ...
  ADEvalCase.Aux[scala.Option[A], Lambda[G => scala.Option[ai.WithGrad[G]]]] =
    new ADEvalCase[scala.Option[A]] with OptionRI[ADEvalMatch, ADEvalCase, A] {
  ...
  override def optionElmInfo[A]: ADEvalCase[scala.Option[A]] => ADEvalCase[A] = _.get(OM[ADEvalMatch, ADEvalCase, A])
}
