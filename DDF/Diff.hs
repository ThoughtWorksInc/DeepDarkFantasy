...
instance Dual r => Dual (Diff r v) where
  ...
  dualGetOrdC :: forall x y. Ord (Diff r v) x :- OrdC (Diff r v) (M.Dual x y)
  dualGetOrdC = Sub (withDict (getOrdC @(Diff r v) @x Proxy) Dict)
...
instance Bimap r => Bimap (Diff r v) where
  ...
  lookupL' = Diff lookupL
  lookupR' = Diff lookupR
  insert' = Diff insert
  updateL' = Diff updateL
...
instance (Vector r v, Lang r) => VTF.VectorTF (Diff r v) where
  ...
  vtfGetOrdC :: forall t f. (Ord (Diff r v) t, Ord (Diff r v) f) :- OrdC (Diff r v) (M.VTF.VectorTF t f)
  vtfGetOrdC = Sub (withDict (getOrdC @(Diff r v) @t Proxy) (withDict (getOrdC @(Diff r v) @f Proxy) Dict))
