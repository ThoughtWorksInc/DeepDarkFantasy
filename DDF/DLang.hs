...
class Lang r => DLang r where
  nextDiff :: (DiffVector r v, RTDiff r x, RTDiff r v) => Proxy v -> r h (InfDiff Eval () x -> InfDiff Eval () (DiffType v x))
  infDiffGet :: RTDiff r x => r h (InfDiff Eval () x -> x)
  rtDiffDiff :: forall v x. (DiffVector r v, RTDiff r v) => Proxy r -> Proxy (v, x) -> RTDiff r x :- RTDiff r (DiffType v x)
